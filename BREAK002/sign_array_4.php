...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $image_name = get_image_name($selected_sign);
}
//画像名を取得
function get_image_name(string $selected_sign):string{
    $sign_names = array(
        ['jp'=>'牡羊座','en'=>'aries'],
        ['jp'=>'牡牛座', 'en'=>'taurus'],
        ['jp'=>'双子座', 'en'=>'gemini'],
        ...
        ['jp'=>'天秤座', 'en'=>'libra'],
        ['jp'=>'蠍座', 'en'=>'scorpio'],
        ['jp'=>'射手座', 'en'=>'sagittarius'],
        ['jp'=>'山羊座', 'en'=>'capricorn'],
        ['jp'=>'水瓶座', 'en'=>'aquarius'],
        ['jp'=>'魚座', 'en'=>'pisces'],
    );
    $image_name = '';
    foreach($sign_names as $array){
        if(in_array($selected_sign,$array)){
            $image_name = $array['en'];
            break;
        }
...
function get_fortune(int $num):string{
    $fortune = '';
    for($i = 0; $i < $num; $i++){
        $fortune .= '★';
    }
    ...
    <div class="container">
        ...
        <h2><?=$current_day?></h2>
            <ul>
                ...
                <li>恋愛運:<?=get_fortune(intval($love))?></li>
                <li>仕事運:<?=get_fortune(intval($job))?></li>
            </ul>
            <?php if($image_name != ''):?>
